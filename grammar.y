...
#line 340 "grammar.y.pre"
		if (!($1 & DECL_MODS)) $1 |= global_modifiers;
		if (!($1 & DECL_ACCESS)) $1 |= DECL_PUBLIC;
		if ($1 & DECL_NOSAVE) {
		...
		/* Handle type checking here so we know whether to typecheck
		   'argument' */
		if ($1 & ~(DECL_MODS|FUNC_VARARGS)) {
		    exact_types = ($1&~(DECL_MODS|FUNC_VARARGS)) | $2;
		} else {
...
#line 368 "grammar.y.pre"
		...
		$<number>$ = FUNC_UNDEFINED | FUNC_PROTOTYPE;
...
#line 396 "grammar.y.pre"
		...
		if ($9) {
		    ...
		    $<number>8 &= ~(FUNC_UNDEFINED | FUNC_PROTOTYPE);
...
#line 2678 "grammar.y.pre"
		 ...
		 */
                ...
                if ($1->type != TYPE_MAPPING && $3->kind == NODE_NUMBER && $3->v.number < 0)
...
#line 3325 "grammar.y.pre"
		...
		$$->kind = NODE_EFUN;
		$$->l.number = $$->v.number + 2;
		$$->v.number = predefs[arrow_efun].token;
#ifdef CAST_CALL_OTHERS
		$$->type = TYPE_UNKNOWN;
#else
                $$->type = TYPE_ANY;
