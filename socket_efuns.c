 ...
 */
static int more_lpc_sockets PROT((void))
{
    int i;
...
int socket_create P3(enum socket_mode, mode, svalue_t *, read_callback, svalue_t *, close_callback)
{
	...
	if (set_socket_nonblocking(fd, 1) == -1) {
	    socket_perror("socket_create: set_socket_nonblocking 1", 0);
	    OS_socket_close(fd);
	    return EENONBLOCK;
	}
	lpc_socks[i].fd = fd;
...
void socket_write_select_handler P1(int, fd)
{
    ...
    if (lpc_socks[fd].flags & S_LINKDEAD)
	return;
